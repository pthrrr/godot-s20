...
A wave-based first-person survival game set on a floating platform in an unknown space. The player must defend against increasingly large waves of hostile vultures while avoiding being pushed off the platform's edges. The game ends when the player falls off the platform, and a highscore leaderboard tracks the best runs.
...
You take stock of your surroundings. The platform is a simple square -- solid ground, but with open edges on every side. One misstep and you fall into the void below.

Then the machines activate. One by one, they begin releasing creatures -- bat-like things with sharp, angular wings. Vultures. They swarm toward you, circling, screeching. Their movements are focused. Aggressive. They are watching you.

Your hands tighten around a weapon you did not notice you were holding.

You fire. Each shot connects. But they keep coming, and they are not trying to hurt you -- they are trying to push you. Their bodies slam into yours, shoving you backward, toward the edges, toward the fall. You dig in. You hold your ground. You survive.

The attack subsides. For a brief moment, everything is quiet again. You catch your breath.

But the calm does not last. The machines activate again. More creatures this time.
...
### 2. Cinematic Intro
A camera flies over and around the level, giving the player an overview of the environment: the platform, the spawners orbiting in the distance, the void below. The camera then swoops down and transitions smoothly into the player's first-person view.

### 3. Player Takes Control
The player now controls the character. The spawners are circling but have not yet activated. The player can look around, move, and get oriented.

### 4. Countdown
A 5-second countdown appears on screen. When the countdown reaches zero:
- The spawners activate and begin spawning vultures
- A "Wave N" announcement appears on screen
- The wave timer starts counting down

### 5. Combat Phase
Vultures rush the player. The player shoots them for points. Vultures push the player on contact (knockback). The player must fight while maintaining position on the platform.

### 6. Wave End
When the wave timer reaches zero:
- Remaining vultures are cleared or flee
- The score for the wave is tallied and added to the total
- Brief calm period before the next wave

### 7. Next Wave
The cycle repeats with increased difficulty (more vultures per wave). Each wave follows the same pattern: countdown, spawners activate, fight, wave ends, calm.
...
- Each wave starts with a 5-second countdown, then spawners activate
- Each wave has a fixed duration (game timer countdown)
- Difficulty scales by increasing the number of vultures spawned per wave
- Between waves there is a brief calm phase

### Knockback
- Creatures do not deal direct damage to the player
- On contact, vultures push the player (knockback force)
...
### Scoring & Highscore
- Each vulture killed awards points
...
- Both players share the same waves and score
...
- **Glass wall barriers**: Transparent walls around the platform that shatter at wave start and rebuild between waves
